              ...
              >
                ...
                <div v-if="msg.senderId === currentUser.id" class="message-sender-info own-avatar">
                  <el-avatar
                      ...
                      size="small"
                      @error="handleSelfAvatarError($event)"
                  />
                </div>
              </div>
            </div>
...
const currentUser = ref({
  id: 'user_123',
  name: '您',
  avatar: 'https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png', // 自己的头像
  online: true
});
...
const openChat = async (contact) => {
  ...
  try {
    ...
    currentMessages.value = res.data.map(msg => ({
      ...msg,
      isRead: msg.receiverId === currentUser.value.id // 消息接收者是当前用户时标记为已读
    }));
